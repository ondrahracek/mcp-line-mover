...
describe("snapshot", () => {
  it("writes manifest + numbered files in op subdir", () => {
    ...
    snapshot("OP1", [join(ws.root, "a.txt"), join(ws.root, "nested/b.txt")], ws.config);
  ...
  it("snapshot bytes are byte-identical to source", () => {
    ws.write("a.txt", "alpha\nwith\nlines\n");
    snapshot("OP2", [join(ws.root, "a.txt")], ws.config);
  ...
  it("snapshot filename is short and does not leak path components", () => {
    ...
    snapshot("OP3", [join(ws.root, "deep/nested/path/with-spaces/file.txt")], ws.config);
  ...
  it("creates the directory lazily and is idempotent on rerun", () => {
    ws.write("a.txt", "v1");
    snapshot("OP4", [join(ws.root, "a.txt")], ws.config);
  ...
  it("snapshot of missing source throws SNAPSHOT_WRITE_FAILED", () => {
    try {
      snapshot("OP5", [join(ws.root, "missing.txt")], ws.config);
  ...
  it("snapshot is rejected for paths outside workspace", () => {
    expect(() => snapshot("OP6", ["/etc/passwd"], ws.config)).toThrow(AppError);
  });
});

describe("snapshotExists", () => {
  it("returns true after snapshot, false otherwise", () => {
    expect(snapshotExists("NONE", ws.config)).toBe(false);
    ws.write("a.txt", "x");
    snapshot("OP7", [join(ws.root, "a.txt")], ws.config);
    expect(snapshotExists("OP7", ws.config)).toBe(true);
  });
});
...
describe("restore", () => {
  it("restores byte-identical content", () => {
    const orig = "alpha\r\nbeta\r\n";
    ws.write("a.txt", orig);
    snapshot("OPR", [join(ws.root, "a.txt")], ws.config);
    writeFileSync(join(ws.root, "a.txt"), "MUTATED");
    restore("OPR", ws.config);
  ...
  it("restores all snapshotted files", () => {
    ws.write("a.txt", "A1");
    ws.write("b.txt", "B1");
    snapshot("OPR2", [join(ws.root, "a.txt"), join(ws.root, "b.txt")], ws.config);
    writeFileSync(join(ws.root, "a.txt"), "A2");
    writeFileSync(join(ws.root, "b.txt"), "B2");
    restore("OPR2", ws.config);
  ...
  it("returns the list of restored absolute paths", () => {
    ws.write("a.txt", "A");
    snapshot("OPR3", [join(ws.root, "a.txt")], ws.config);
    const restored = restore("OPR3", ws.config);
    expect(restored).toEqual([join(ws.root, "a.txt")]);
  });

  it("throws when snapshot directory missing", () => {
    expect(() => restore("MISSING", ws.config)).toThrow(AppError);
  });
});
