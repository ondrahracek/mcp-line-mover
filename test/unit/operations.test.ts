...
function baseInput(): Omit<OperationRecord, "operation_id" | "created_at" | "updated_at" | "status"> {
  return {
    description: "test op",
    source_path: "src.txt",
    start_line: 1,
...
describe("createOperation + loadOperation", () => {
  it("persists record under .mcp-line-mover/operations/<id>.json", () => {
    const op = createOperation(baseInput(), ws.config);
  ...
  it("loadOperation round-trips all fields", () => {
    const op = createOperation(baseInput(), ws.config);
    const loaded = loadOperation(op.operation_id, ws.config);
    expect(loaded).toEqual(op);
  });

  it("loadOperation throws OPERATION_NOT_FOUND for missing id", () => {
    try {
      loadOperation("NOSUCH", ws.config);
  ...
  it("survives across config instances (process restart simulation)", () => {
    ...
    const loaded = loadOperation(op.operation_id, cfg2);
...
describe("updateOperation", () => {
  ...
  it("rejects optimistic check failure (concurrent update)", () => {
    const op = createOperation(baseInput(), ws.config);
    updateOperation(op.operation_id, op.updated_at, { status: "executed" }, ws.config);
    try {
      updateOperation(
        op.operation_id,
        op.updated_at,
  ...
  it("rejects backward transitions", () => {
    const op = createOperation(baseInput(), ws.config);
    const e1 = updateOperation(op.operation_id, op.updated_at, { status: "executed" }, ws.config);
    const u1 = updateOperation(op.operation_id, e1.updated_at, { status: "undone" }, ws.config);
    expect(() =>
      updateOperation(op.operation_id, u1.updated_at, { status: "executed" }, ws.config),
    ).toThrow(AppError);
  });
});
...
describe("TTL / expiry", () => {
  it("loadOperation returns expired status when older than TTL", () => {
    ...
    const op = createOperation(baseInput(), cfg);
    const file = join(ws.root, ".mcp-line-mover", "operations", `${op.operation_id}.json`);
    const stored = JSON.parse(readFileSync(file, "utf8"));
    stored.created_at = new Date(Date.now() - 3 * 24 * 3600 * 1000).toISOString();
    writeFileSync(file, JSON.stringify(stored));
    const loaded = loadOperation(op.operation_id, cfg);
  ...
  it("does not delete the file on expiry", () => {
    ...
    stored.created_at = new Date(Date.now() - 3 * 24 * 3600 * 1000).toISOString();
    writeFileSync(file, JSON.stringify(stored));
    loadOperation(op.operation_id, cfg);
    expect(existsSync(file)).toBe(true);
  });
});
...
describe("listOperations", () => {
  it("returns all operations sorted by updated_at desc", async () => {
    const a = createOperation(baseInput(), ws.config);
    await new Promise((r) => setTimeout(r, 5));
    const b = createOperation(baseInput(), ws.config);
    const list = listOperations(ws.config);
  ...
  it("ignores non-json files in the operations dir", () => {
    createOperation(baseInput(), ws.config);
  ...
  it("returns empty array when dir does not exist", () => {
    expect(listOperations(ws.config)).toEqual([]);
  });
});
